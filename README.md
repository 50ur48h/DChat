...
flowchart LR
    ...
    subgraph GATE["Deterministic checks — ordinary code, no AI"]
...
The AI proposes. Code disposes. Every one of those checks is ordinary code with
a test behind it, and **none of them can be talked out of by a cleverly worded
...
**The language model is outside the trust boundary.** It is sent a description of
your tables and the rows a query returned. It is never given a credential, never
given a connection, and never asked a question whose answer decides whether
...
- **It cannot answer what the data cannot support.** If the table you would need
  ...
  tell you that rather than approximating.
...
| | |
|---|---|
| **[Run it yourself](docs/setup.md)** | Installation, a demo database, and your first question. About ten minutes. |
| **[Architecture](docs/architecture.md)** | The full technical design. |

Supported databases today: **PostgreSQL** and **SQL Server**. Sign-in works with
your existing corporate identity provider. Each organisation's data is separated
by the database itself, and people can be given Admin, Contributor or Reader
access.
...
None yet. Until a license is chosen, the default applies: all rights reserved.
